...
`harrier` is the text layer that sits directly above `redwing`.  Where `redwing` operates purely at
the level of raw bytes — tracking deltas, managing piece tables, and providing non-destructive change
management — `harrier` introduces the concepts that are meaningful for *text*:
...
- **Character encoding.** Transparent decoding and re-encoding of byte regions using a declared
  ...
  arithmetic inside `harrier` is always performed in bytes; decoded character views are derived
...
`harrier` wraps a `redwing` branch and adds the text-specific layer on top:

```
┌─────────────────────────────────────────────┐
│                   harrier                    │  ← line map, encoding, red-green tree
├─────────────────────────────────────────────┤
...
When a text edit is made through `harrier`, it computes the affected byte range (accounting for the
...
`harrier` takes a different approach:

| Concept in `rowan`               | Concept in `harrier`                                      |
|----------------------------------|----------------------------------------------------------|
...
`harrier` does not aim to be a drop-in replacement for `rowan`; the API will differ where necessary
to honour source encodings and to expose the richer change-management model that `redwing` provides.

## Input Model

`harrier` takes its byte data from a `redwing` `Branch` — specifically an `Arc<dyn Branch>`.  The
choice of `Branch` rather than `Thicket` is deliberate:

- A `Thicket` is an **owner** of the first writable branch and the root of a fork tree.  `harrier`
...
- An `Arc<dyn Branch>` is the natural handle for that: it allows multiple consumers to share the
  same branch, and the interior-mutability of `DerivedBranch` means writes from `harrier` are
  recorded as deltas without requiring `harrier` to hold exclusive access.

In addition to the branch, `harrier` accepts an optional **byte range** `[start, end)` within the
branch.  This allows `harrier` to treat an embedded text region inside a larger binary file as its
...
`harrier` **must not require the full byte content to be resident in memory at once.**  The
`Branch` trait provides `read_at(offset, buf)` and `as_reader()` — both are lazy, demand-driven
interfaces that read only as many bytes as the caller requests.  `harrier` uses these exclusively
for scanning:
...
`harrier` is designed to serve two distinct but closely related consumers.  The two modes differ
...
needs none of harrier's line abstraction — it manages its own tree structure — but it does need:
...
- **Re-encoding for insertion.** When the parser crate wants to insert or replace text, it
  has Unicode strings; harrier must re-encode them to the source encoding's byte sequences.
...
**Design intent.** `harrier`'s client-facing abstraction presents text as a sequence of *lines* —
...
1. **How is a new line break encoded? — Resolved.**  The byte sequence used when inserting a
   ...
   **Detection — Resolved.** `harrier` scans the probe buffer (the same up-to-8 KiB block
...
5. **Inserting line-break characters through the content API — Resolved.**  If a caller
   ...
   replacement string contains any character or byte sequence that `harrier` would recognise as
   ...
   **The prohibition is self-referential by design.**  The set of sequences that are illegal
   to insert through the content API is exactly the set of sequences that `harrier` recognises
   as line terminators.  There is no separate list to maintain; the two are defined together.
   If `harrier`'s recognised line-ending set ever changes, the content-API prohibition changes
   with it automatically and consistently.

### Multi-Line Regex and the Normalised Logical View

`harrier` supports multi-line regular expressions by providing a **normalised view** of the
branch content.  Because `harrier` hides line terminators from clients (see *Line Ending
...
**harrier is not a regex runner.** The caller owns the regex engine, constructs the pattern, and
drives the match loop.  harrier's role is to provide the contiguous normalised buffer the regex
...
at materialisation time.  Single-line patterns do not require a full materialisation; harrier can
...
not known until the entire file has been scanned.  harrier exposes line count as a value paired
...
**Background scan and eventing.** harrier does not own threads.  To improve the estimate
...
than a registered callback, so harrier is decoupled from any particular threading or async model.
...
which encoding is in use; harrier is the complete encoding boundary.
...
`harrier` uses [`encoding_rs`](https://crates.io/crates/encoding_rs) for decoding and re-encoding.
Decoded `String` values produced by `harrier` are always Rust-native UTF-8; the encoding is an
attribute of the *storage representation*, not of the in-memory API.  Re-encoding uses the same
declared encoding: when a caller writes new text into a node, `harrier` re-encodes it in the source
...
decoder and encoder in `encoding_rs`.  However, `harrier` must validate at open time that the
...
Rather than requiring the caller to know the source encoding in advance, `harrier` detects it
automatically at open time.  The detection pipeline is:

1. **Probe read.** `harrier` reads up to a configurable number of bytes from the start of the
   source (default: 8 192 bytes) into a small local buffer.  This is the *only* speculative
   buffering `harrier` ever does; the probe buffer is discarded once the encoding is chosen.
...
`chardetng` is the default but is not mandatory.  `harrier` defines an `EncodingDetector` trait
...
When a BOM is detected, `harrier` records its byte span.  Two behaviours are then possible:
...
commits to it; there is no "low confidence" signal that `harrier` can act on.  If that guess is
...
- **Same size or smaller.** If every replacement produces no more bytes than it consumes,
  ...
  database-within-a-file storage — the problem is not esoteric, it is fundamental.  `harrier`
...
The granularity of the green tree is left to the consumer: `harrier` defines the mechanism but not
...
`harrier` operates under an explicit memory discipline:
...
- **No single internal allocation may exceed a compile-time constant.**  The target ceiling is
  ...
  but a design constraint: no data structure or algorithm inside `harrier` is permitted to
...
- **Caller-requested materialisation is the caller's responsibility.**  If a caller asks
  `harrier` for a `Vec<u8>` of the full processed output, `harrier` will attempt to produce it.
  The decision to materialise an arbitrarily large buffer belongs to the caller, not to
  `harrier`; `harrier` will not refuse the request on memory grounds, but it will not buffer
...
- **Storage offloading for large modified content is an open question.**  For very large files
  ...
  (or the materialised output) to temporary storage.  Whether that mechanism lives in `harrier`,
